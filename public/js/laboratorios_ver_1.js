...
async function loadPrediosSelect() {
    try {
        ...
        if (!select) {
            console.error('Elemento #predio_id não encontrado no DOM. Verifique o HTML do formulário.');
        ...
        activePredios.forEach(predio => {
            ...
            option.text = `${predio.prd_num} - ${predio.prd_nom}`;
...
async function editLaboratorio(id) {
    try {
        ...
        // Aguarde o modal estar renderizado
        setTimeout(async () => await loadPrediosSelect(), 100);
...
document.getElementById('laboratorioForm')?.addEventListener('submit', async (e) => {
    ...
    data.predio_id = parseInt(data.predio_id);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    modalElement.addEventListener('show.bs.modal', async (event) => {
        if (event.relatedTarget && event.relatedTarget.id === 'newLaboratorioBtn') {
            ...
            // Aguarde o modal estar totalmente renderizado
            setTimeout(async () => await loadPrediosSelect(), 100);
            //await loadPrediosSelect();
        }
    });
    ...
    loadUserGreeting();
    loadLaboratorios();
    // loadPrediosSelect(); // Carregar apenas no modal
});
