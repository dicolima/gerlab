...
class ReservaController {
  static async consultarReservas(req, res) {
    ...
    try {
      ...
      const dateObj = new Date(datah + 'T00:00:00-03:00');
      ...
      for (const lab of laboratorios) {
        ...
        for (const hora of horarios) {
          ...
          const horaFim = `${(parseInt(hora.split(':')[0], 10) + 1).toString().padStart(2, '0')}:00:00`;
            ...
            if (new Date(datah).toISOString().split('T')[0] >= new Date(data_inicio).toISOString().split('T')[0] &&
              ...
              if (inicioReserva <= fimHora && fimReserva >= inicioHora) {
                if (labStatus === 'Falta') {
                  ...
                  cellContent = `${pro_nom} ${pro_sob}<br>${dis_nom || 'Sem disciplina'}`;
                }
                break;
              }
            }
  ...
  static async getListaPresenca(req, res) {
    const { datah, semestre } = req.body;

    if (!datah || !semestre) {
      return res.status(400).json({ error: 'Data e semestre são obrigatórios' });
    }

    try {
      const reservas = await ReservaModel.getListaPresenca(datah);
      res.status(200).json({ reservas, semestre });
    } catch (error) {
...
module.exports = ReservaController; // EM TESTE PARA IMPRIMIR LISTA
